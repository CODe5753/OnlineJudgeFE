<template>
  <Row type="flex">
    ...
    <Panel id="contest-card" shadow>
      <div slot="title">{{query.rule_type === '' ? this.$i18n.t('전체') : query.rule_type}} {{$t('대회')}}</div>
      <div slot="extra">
        <ul class="filter">
          <li>
      ...
      <p id="no-contest" v-if="contests.length == 0">{{$t('대회가 없습니다')}}</p>
      <ol id="contest-list">
    ...
    methods: {
      ...
      goContest (contest) {
        ...
        if (contest.contest_type !== CONTEST_TYPE.PUBLIC && !this.isAuthenticated) {
          this.$error(this.$i18n.t('로그인이 필요합니다.'))
